...
@jax.jit
def _my_harvesting_reward_fn(prev_obs: jnp.ndarray, current_obs: jnp.ndarray, done: jnp.ndarray) -> jnp.ndarray:
    """
...
@jax.jit
def _my_crafting_reward_fn(prev_obs_flat: jnp.ndarray, current_obs_flat: jnp.ndarray, done: jnp.ndarray) -> jnp.float32:
    """
...
@jax.jit
def _my_survival_reward_fn(prev_obs, obs, done):
    """
...
def calculate_all_skill_rewards(prev_obs, current_obs, done):
    """
    Calculate rewards for all three skills simultaneously.
    ...
    Returns:
        tuple: (harvest_reward, craft_reward, survival_reward)
    """
    harvest_reward = _my_harvesting_reward_fn(prev_obs, current_obs, done)
    craft_reward = _my_crafting_reward_fn(prev_obs, current_obs, done)
    survival_reward = _my_survival_reward_fn(prev_obs, current_obs, done)
    
    return harvest_reward, craft_reward, survival_reward
