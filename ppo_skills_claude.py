...
from reward_fns.skill_rewards import (
    get_vmap_compatible_reward_fn,
)
...
def make_train(config):
    ...
    # CLAUDE REWARD
    vmapped_reward_fn, init_state = get_vmap_compatible_reward_fn()
    ...
    def train(rng):
        ...
        # Initialize skills - one per environment
        rng, _rng = jax.random.split(rng)
        ...
        prev_inventories, first_steps = init_state(config["NUM_ENVS"])

        def augment_obs_with_skill(obsv, skill_vec):
            return jnp.concatenate([obsv, skill_vec])
        ...
        def _update_step(runner_state, unused):
            ...
            def _env_step(runner_state, unused):
                ...
                # reward_i = jax.vmap(get_harvester_reward)(base_obsv)
                rewards, prev_inventories, first_steps = vmapped_reward_fn(base_obsv, prev_inventories, first_steps)
                reward_i = rewards
...
if __name__ == "__main__":
    ...
    # SKILLS
    parser.add_argument("--max_num_skills", type=int, default=3)
